 ...
 * generally more efficient to read a set of values with a single request, the batch read by default will assume that 
 * no such error responses will be returned. If your batch request results in such errors, it is recommended that you 
...
public class BatchRead<K> {
    ...
    /**
     * If this value is false, any error response received will cause an exception to be thrown, and the entire
     * batch to be aborted. If set to true, error responses will be set as the result of all affected locators and the
     * entire batch will be attempted with no such exceptions thrown.
     */
    ...
    private void doPartition() {
        Map<SlaveAndRange, List<KeyedModbusLocator<K>>> slaveRangeBatch =
                new HashMap<SlaveAndRange, List<KeyedModbusLocator<K>>>();
        ...
        for (List<KeyedModbusLocator<K>> functionLocatorList : functionLocatorLists) {
            ...
            if (contiguousRequests)
                ...
                // this function
                int maxReadCount = RegisterRange.getMaxReadCount(
                        functionLocatorList.get(0).getSlaveAndRange().getRange());
     ...
     * anything. We pretty much just try to fit as many locators as possible into a single valid request, and 
     * then move on.
     * 
    ...
    private class FunctionLocatorComparator implements Comparator<KeyedModbusLocator<K>> {

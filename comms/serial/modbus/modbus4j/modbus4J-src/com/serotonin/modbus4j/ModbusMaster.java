...
abstract public class ModbusMaster extends Modbus {
     ...
     */
    public Object getValue(int slaveId, int registerId, int dataType)
     ...
     * Convenience method. See getValue(ModbusLocator)
     */
    public Object getValue(int slaveId, int range, int offset, int dataType)
            throws ModbusTransportException, ErrorResponseException {
     ...
     * Convenience method. See getValue(ModbusLocator)
     */
    public Boolean getValue(int slaveId, int registerId, byte bit)
            throws ModbusTransportException, ErrorResponseException {
        RangeAndOffset rao = new RangeAndOffset(registerId);
     ...
     * Convenience method. See getValue(ModbusLocator)
     */
    public Boolean getValue(int slaveId, int range, int offset, byte bit)
            throws ModbusTransportException, ErrorResponseException {
     ...
     * allowed to be requested including multi-word types. The determination of the correct request message to send 
     ...
     */
    public void setValue(int slaveId, int registerId, int dataType, Object value)
            throws ModbusTransportException, ErrorResponseException {
        RangeAndOffset rao = new RangeAndOffset(registerId);
     ...
     * Convenience method. See setValue(ModbusLocator, Object)
     */
    public void setValue(int slaveId, int registerId, byte bit, boolean value)
            throws ModbusTransportException, ErrorResponseException {
        RangeAndOffset rao = new RangeAndOffset(registerId);
     ...
     * Convenience method. See setValue(ModbusLocator, Object)
     */
    public void setValue(int slaveId, int range, int offset, byte bit, boolean value)
            throws ModbusTransportException, ErrorResponseException {
     ...
     * allowed to be set including including multi-word types. The determination of the correct write message to send 
     * is handled automatically.
     *  
     * @param locator the information required to locate the value in the modbus network.
     * @value an object representing the value to be set. This will be one of Boolean, Short, Integer, Long, 
     *   BigInteger, Float, or Double. See the DataType enumeration for details on which type to expect. 
     * @throws ModbusTransportException if there was an IO error or other technical failure while sending the message
     * @throws ErrorResponseException if the response returned from the slave was an exception.
     */
     ...
     * and in any case the results would not be meaningful since there would be no semantic information accompanying 
     * the results.
     */
    ...
    public <K> BatchResults<K> send(BatchRead<K> batch) throws ModbusTransportException, ErrorResponseException {
        ...
        for (ReadFunctionGroup<K> functionGroup : functionGroups)
            sendFunctionGroup(functionGroup, results, batch.isErrorsInResults());
    ...
    //
    /**
     * This method assumes that all locators have already been pre-sorted and grouped into valid requests,
     * say, by the createRequestGroups method. 
     */
    private <K> void sendFunctionGroup(ReadFunctionGroup<K> functionGroup, BatchResults<K> results,
            boolean errorsInResults) throws ModbusTransportException, ErrorResponseException {
        ...
        if (functionGroup.getFunctionCode() == FunctionCode.READ_COILS)
            ...
            request = new ReadHoldingRegistersRequest(slaveId,
                functionGroup.getStartOffset(), functionGroup.getLength());
        ...
        ReadResponse response = (ReadResponse)send(request);
        byte[] data = null;
            ...
            if (errorsInResults && response.isException())
                ...
                try {
                    ...
                    throw new RuntimeException("Result conversion exception. data="+ ArrayUtils.toHexString(data) +
                            ...
                            ", locator.endOffset="+ locator.getEndOffset() +", locator.length="+ locator.getLength() +
                            ", locator.offset="+ locator.getOffset() +", locator.bit="+ locator.getBit() +
                            ", locator.key="+ locator.getKey() +", locator.bit.class="+ locator.getKey().getClass() +
                            ", functionGroup.functionCode="+ functionGroup.getFunctionCode() +
                            ", functionGroup.startOffset="+ functionGroup.getStartOffset() +
                            ", functionGroup.length="+ functionGroup.getLength(), e);
                }
            }
        }
    }

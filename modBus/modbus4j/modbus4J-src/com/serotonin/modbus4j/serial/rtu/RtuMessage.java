package com.serotonin.modbus4j.serial.rtu;

import com.serotonin.io.messaging.Message;
import com.serotonin.modbus4j.base.ModbusUtils;
import com.serotonin.modbus4j.exception.ModbusTransportException;
import com.serotonin.modbus4j.msg.ModbusMessage;
import com.serotonin.util.queue.ByteQueue;

/**
 * Convenience superclass primarily for calculating CRC values.
 * @author mlohbihler
 */
public class RtuMessage implements Message {
    protected ModbusMessage modbusMessage;
    private boolean reverseCRC;
    
    public RtuMessage(ModbusMessage modbusMessage, boolean reverseCRC) {
        this.modbusMessage = modbusMessage;
        this.reverseCRC = reverseCRC;
    }
    
    public boolean isReverseCRC() {
        return reverseCRC;
    }
    
    public byte[] getMessageData() {
        ByteQueue queue = new ByteQueue();
        
        // Write the particular message.
        modbusMessage.write(queue);
        
        // Write the CRC
        ModbusUtils.pushShort(queue, calculateCRC());
        
        // Return the data.
        return queue.popAll();
    }
    
    protected void checkCRC(ByteQueue queue) throws ModbusTransportException {
        // Check the CRC
        int calcCrc = calculateCRC();
        int givenCrc = ModbusUtils.popUnsignedShort(queue);
        
        if (calcCrc != givenCrc)
            throw new ModbusTransportException("CRC mismatch: given="+ givenCrc +", calc="+ calcCrc);
    }
    
    private int calculateCRC() {
        ByteQueue queue = new ByteQueue();
        modbusMessage.write(queue);
        
        int high = 0xff;
        int low = 0xff;
        int nextByte = 0;
        int uIndex;
        
        while (queue.size() > 0) {
            nextByte = 0xFF & queue.pop();
            uIndex = high ^ nextByte;
            high = low ^ lookupCRCHi[uIndex];
            low = lookupCRCLo[uIndex];
        }

        // Determine end order of bytes.
        if (reverseCRC)
            return (low << 8) | high;
        return (high << 8) | low;
    }
    
    // Table of CRC values for high-order byte
    private final static short[] lookupCRCHi = {
        
    };

    // Table of CRC values for low-order byte
    private final static short[] lookupCRCLo = {
        
    };
}
